# Speichern Attribut "_id" and "setences" im Datensatz in Tabelle "2wikimultihop_kontext"
import pymysql
import json
import os

# Erstellung der Tabelle "2wikimultihop_kontext"
def prem(db):
  cursor = db.cursor()
  cursor.execute("SELECT VERSION()")
  data = cursor.fetchone()
  print("Database version : %s " % data) # Überprüfen, ob die Verbindung erfolgreich hergestellt wurde.
  cursor.execute("DROP TABLE IF EXISTS 2wikimultihop_kontext")
  sql = """CREATE TABLE 2wikimultihop_kontext (
       id CHAR(50) NOT NULL,
                ...
                while True:
                    try:
                        ...
                        for i in text:
                            ...
                            print(daten)
                            insert_re = "insert into 2wikimultihop_kontext(id, context) values (%s, %s)"
                            cursor = db.cursor()
